...
// Public SVGs are copies of the source vectors, just like the raster exports.
for (const fileName of (await NodeFSP.readdir(NodePath.join(root, "assets/gusty"))).toSorted()) {
  if (!fileName.endsWith(".svg")) continue;
  const contents = await NodeFSP.readFile(NodePath.join(root, "assets/gusty", fileName));
  for (const directory of ["apps/web/public/brand", "apps/marketing/public/brand"]) {
    outputs.set(`${directory}/${fileName}`, contents);
  }
}
