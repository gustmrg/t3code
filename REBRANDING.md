...
O fluxo de integração e releases está em [Manutenção do Gusty Code](docs/operations/gusty-code-updates.md).

- `packages/shared/src/branding.ts`: nome de exibição compartilhado pelos pontos de entrada dos clientes.
- `packages/shared/src/gustyTheme.ts`: paletas e definição do tema Gusty; o motor continua em `themePalette.ts`.
- `assets/gusty/`: vetores e referência do design system; os assets são exportados por `scripts/export-gusty-icons.ts`.
- Metadados estáticos de instalação, HTML, CSS mobile e textos editoriais continuam específicos de cada superfície.
- `environmentIdentificationMode` permanece no contrato para preservar preferências existentes. O padrão é `pill`; a sidebar Gusty mantém sua apresentação fixa.
- Kimi foi removido. Os planos 001 e 002 são somente histórico e não devem ser reaplicados.

## 1. Assets-fonte da marca

No Gusty Code, os vetores de `assets/gusty/` são a fonte dos exports feitos por `vp run icons:export`. Os projetos do Icon Composer abaixo continuam presentes para as superfícies que os utilizam:
...
Após alterar os vetores de `assets/gusty/`:

1. Executar `vp run icons:export`.
2. Executar `vp run icons:check`, que também confere as cópias SVG públicas.
3. Não editar diretamente PNGs, ICOs ou cópias SVG geradas.

Se alterar um projeto nativo do Icon Composer, usar o fluxo específico de
`vp run icons:export:composer`, incluindo a exportação manual quando solicitada.

Referências:

- [assets/README.md](assets/README.md)
- [scripts/export-gusty-icons.ts](scripts/export-gusty-icons.ts)
- [scripts/export-brand-icons.ts](scripts/export-brand-icons.ts) (Icon Composer)
