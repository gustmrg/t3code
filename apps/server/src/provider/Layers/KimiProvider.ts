...
import {
  deleteTemporaryKimiSession,
  discoverKimiModelsFromRuntime,
  KIMI_MINIMUM_ACP_VERSION,
  makeKimiAcpRuntime,
  ...
  return Effect.gen(function* () {
    ...
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "warning",
        auth: { status: "unknown" },
        message: "Checking Kimi Code availability...",
      },
    });
  });
}
...
const discoverKimiModelsViaAcp = (
  ...
  Effect.gen(function* () {
    ...
    return yield* discoverKimiModelsFromRuntime(acp).pipe(
      Effect.ensuring(deleteTemporaryKimiSession(acp, started.sessionId)),
    );
  }).pipe(Effect.scoped);

const isKimiAuthRequiredCause = (cause: Cause.Cause<unknown>): boolean => {
  const failure = Cause.findErrorOption(cause);
  return (
    Option.isSome(failure) &&
    Schema.is(EffectAcpErrors.AcpRequestError)(failure.value) &&
    failure.value.code === -32000
  );
};
...
> {
  ...
  if (!version) {
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "error",
        auth: { status: "unknown" },
        message: `Could not determine the Kimi Code version. Version ${KIMI_MINIMUM_ACP_VERSION} or newer is required.`,
      },
    });
  }

  if (compareSemverVersions(version, KIMI_MINIMUM_ACP_VERSION) < 0) {
    return buildServerProvider({
      presentation: KIMI_PRESENTATION,
      enabled: kimiSettings.enabled,
      checkedAt,
      ...
      probe: {
        ...
        status: "error",
        auth: { status: "unknown" },
        message: `Kimi Code v${version} is too old. Upgrade to v${KIMI_MINIMUM_ACP_VERSION} or newer.`,
      },
    });
  }
  ...
  if (Exit.isFailure(discoveryExit)) {
    ...
    const authRequired = isKimiAuthRequiredCause(discoveryExit.cause);
    return buildServerProvider({
      presentation: KIMI_PRESENTATION,
      enabled: kimiSettings.enabled,
      checkedAt,
      ...
      probe: {
        installed: true,
        version,
        status: authRequired ? "warning" : "error",
        auth: { status: authRequired ? "unauthenticated" : "unknown" },
        message: authRequired
          ? "Kimi Code is not authenticated. Run `kimi login` on the T3 server environment and try again."
          : "Kimi Code is installed but ACP startup failed. Check server logs for details.",
      },
    });
  }
  ...
  return buildServerProvider({
    ...
    probe: {
      ...
      version,
      status: "ready",
      auth: { status: "authenticated", type: "login" },
    },
  });
});
