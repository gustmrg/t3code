...
export type KimiPermissionRequestKind = "standard" | "question" | "plan" | "unknown";

export function classifyKimiPermissionRequest(
  request: EffectAcpSchema.RequestPermissionRequest,
): KimiPermissionRequestKind {
  const ids = request.options.map((option) => option.optionId.trim());
  if (ids.length === 0) return "unknown";
  if (ids.every((id) => /^q\d+_(?:opt_\d+|skip)$/.test(id))) return "question";
  if (
    ids.every(
      (id) =>
        /^plan_opt_\d+$/.test(id) ||
...
export function kimiPermissionQuestions(
  request: EffectAcpSchema.RequestPermissionRequest,
  kind: "question" | "plan",
  ...
  return [
    {
      ...
      header: kind === "plan" ? "Plan review" : "Question",
      question:
        request.toolCall.title?.trim() ||
        (kind === "plan" ? "Choose how to proceed with this plan." : "Choose an answer."),
      multiSelect: false,
...
function answerLabels(answers: ProviderUserInputAnswers): ReadonlyArray<string> {
...
export function selectKimiUserInputOptionId(
  request: EffectAcpSchema.RequestPermissionRequest,
  answers: ProviderUserInputAnswers,
): string | undefined {
  const labels = answerLabels(answers);
  return request.options.find((option) => labels.includes(option.name))?.optionId;
}
  ...
  const fallbackKind =
    decision === "acceptForSession"
      ...
      : decision === "accept"
        ...
        : "reject_once";
  const option =
    request.options.find((entry) => preferredIds.includes(entry.optionId)) ??
...
function selectAutoApprovedPermissionOption(
  ...
  return (
    selectKimiPermissionOptionId(request, "acceptForSession") ??
    selectKimiPermissionOptionId(request, "accept")
  );
}
    ...
    const startSession: KimiAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
            ...
            yield* acp.handleRequestPermission((params) =>
              mapAcpCallbackFailure(
                Effect.gen(function* () {
                  ...
                  if (requestKind === "unknown") {
                    return { outcome: { outcome: "cancelled" as const } };
                  }
                  if (requestKind === "question" || requestKind === "plan") {
                    ...
                    yield* offerRuntimeEvent({
                      ...
                      payload: { questions: kimiPermissionQuestions(params, requestKind) },
                      raw: {
                        source: "acp.jsonrpc",
                    ...
                    const selectedOptionId =
                      resolved._tag === "answered"
                        ? selectKimiUserInputOptionId(params, resolved.answers)
                        : undefined;
                    return {
                  ...
                  const selectedOptionId =
                    resolved === "cancel"
                      ? undefined
                      : selectKimiPermissionOptionId(params, resolved);
                  return {
                    outcome: selectedOptionId
                      ? {
                          outcome: "selected" as const,
          ...
          const boundModelId = currentKimiModelFromConfigOptions(yield* acp.getConfigOptions);
        ...
        const prepared = yield* withThreadLock(
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const currentModelId = currentKimiModelFromConfigOptions(
